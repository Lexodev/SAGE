...
/**
 * GT car :
 *   - Move with keypad keys
 *   - Rest with space
 *   - Moive camera with mouse
 *   - Quit with ESC
 */
...
VOID _Render(VOID)
{
  ...
  SAGE_RenderWorld();
  // Draw the angles
  SAGE_PrintFText(10, 15, "CAM AX=%d  AY=%d / CAR AX=%d  AY=%d AZ=%d", cax, cay, vax, vay, vaz);
