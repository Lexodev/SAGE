...
void main(void)
{
  ...
  SAGE_AppliLog("--------------------------------------------------------------------------------");
  SAGE_AppliLog("    SAGE library 3D test (3DEOPTIMIZE) / %s", SAGE_GetVersion());
  SAGE_AppliLog("--------------------------------------------------------------------------------");
  if (SAGE_Init(SMOD_VIDEO|SMOD_3D)) {
    ...
    if (SAGE_OpenScreen(SCREEN_WIDTH, SCREEN_HEIGHT, SCREEN_DEPTH, SSCR_STRICTRES)) {
      ...
      if ((entity = SAGE_LoadEntity("data/bad_cube.obj")) != NULL && SAGE_OptimizeEntity(entity) && SAGE_AddEntity(CUBE_ENTITY, entity)) {
        ...
        SAGE_SetEntityPosition(CUBE_ENTITY, (FLOAT)0.0, (FLOAT)0.0, (FLOAT)50.0);
        ...
        while (!finish) {
          ...
          SAGE_ClearScreen();
          SAGE_RotateEntity(CUBE_ENTITY, S3DE_ONEDEGREE, S3DE_ONEDEGREE, S3DE_ONEDEGREE);
          SAGE_RenderWorld();
