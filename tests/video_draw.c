...
void dump_coords(STRPTR item, LONG *crd, ULONG nb)
{
  ...
  SAGE_AppliLog("** Dump '%s' line coords (%d) **", item, nb);
  index = 0;
  nb++;
  while (nb--) {
...
void test_coords(void)
{
  ...
  SAGE_AppliLog("** Horizontal line - coords 115,50 -> 10,50 **");
  ...
  SAGE_AppliLog("** Diagonal line - coords 96,10 -> 10,96 **");
  ...
  SAGE_AppliLog("** Soft slope line - coords 250,10 -> 80,98 **");
  ...
  SAGE_AppliLog("** Hard slope line - coords 60,10 -> 10,93 **");
  ...
  /*SAGE_AppliLog("** Soft slope line - coords 250,10 -> 80,44 **");
